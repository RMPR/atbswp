//! XDG desktop portal calls over libdbus (loaded at run time).  Mirrors the
//! C implementation in `player/src/backend_linux.c`.
...
impl Portal {
    ...
    pub fn request(
        ...
        let mut args = DBusMessageIter([0; 16]);
        let mut dict = DBusMessageIter([0; 16]);
        unsafe {
        ...
        let mut rit = DBusMessageIter([0; 16]);
        unsafe {
    ...
    unsafe fn dict_add(&self, dict: &mut DBusMessageIter, key: &str, v: &Value) {
        let d = &self.d;
        let mut entry = DBusMessageIter([0; 16]);
        let mut var = DBusMessageIter([0; 16]);
        let k = cs(key);
        let kp = k.as_ptr();
        unsafe {
    ...
    fn parse_response(&self, m: Msg) -> Result<(u32, Vec<(String, Value)>), String> {
        let d = &self.d;
        let mut it = DBusMessageIter([0; 16]);
        unsafe {
    ...
    unsafe fn read_value(&self, it: &mut DBusMessageIter) -> Value {
        ...
        unsafe {
            match (d.dbus_message_iter_get_arg_type)(it) {
                ...
                T_STRUCT => {
                    let mut s = DBusMessageIter([0; 16]);
